...
describe('StateStoreService', () => {
  ...
  describe('deve testar clearAllStorages', () => {
    it('deve chamar o método clearAllStorages do StorageService', () => {
      const storageServiceSpy = vi.spyOn(service['storageService'], 'clearAllStorages');

      service.clearAllStorages();
  ...
  describe('deve testar state$', () => {
    ...
    it('deve testar o state$ com state definido', () => {
      const definedState: State = {
        data: 'estado definido',
        dataTest: 'teste definido',
      };

      service['valueState'] = {
        data: 'estado inicial',
        dataTest: 'teste inicial',
      };

      service['stateStore$'] = scheduled([definedState], asyncScheduler);

      return new Promise<void>((resolve) => {
        service.state$.subscribe((state) => {
          expect(state).toEqual(definedState);
          resolve();
        });
      });
    });
